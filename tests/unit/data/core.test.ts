...
describe("Notes", () => {
    ...
    describe("Review New note (i.e. not previously reviewed); no questions present", () => {
        test("Review GAOKAO note as Again preserves unknown YAML and Chinese content", async () => {
            const settings: SRSettings = { ...DEFAULT_SETTINGS };
            const osrCore: UnitTestOsrCore = new UnitTestOsrCore(settings);
            await osrCore.loadTestVault("gaokao-note-again");

            const file = osrCore.getFileByNoteName("导数判断单调性");
            const bodyBefore = file.content.replace(/^---\r?\n[\s\S]*?\r?\n---\r?\n?/, "");

            await osrCore.saveNoteReviewResponse(file, ReviewResponse.Again, settings);

            unitTestCheckNoteFrontmatter(file.content, "2023-09-06", 0, 230);
            expect(file.content).toContain("gaokao_id: math-calculus-monotonicity-001");
            expect(file.content).toContain("custom_unknown: 必须保留");
            expect(file.content.replace(/^---\r?\n[\s\S]*?\r?\n---\r?\n?/, "")).toEqual(bodyBefore);
        });

        test("Reviewing a note created after the initial load adds it to the existing queue", async () => {
            const settings: SRSettings = { ...DEFAULT_SETTINGS };
            const osrCore: UnitTestOsrCore = new UnitTestOsrCore(settings);
            await osrCore.loadTestVault("filesButNoQuestions");
            ...
            const file = new UnitTestSRFile(
                [
                    "---",
                    "gaokao_id: chemistry-knowledge-created-after-load",
            ...
            const reviewDeck = osrCore.noteReviewQueue.reviewDecks.get("#review");
            expect(reviewDeck?.newNotes).toHaveLength(0);
            expect(reviewDeck?.scheduledNotes).toHaveLength(1);
            expect(reviewDeck?.scheduledNotes[0].note.path).toBe(file.path);
            expect(osrCore.noteReviewQueue.dueNotesCount).toBe(1);
