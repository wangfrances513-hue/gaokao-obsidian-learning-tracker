...
export class NoteReviewQueue {
    ...
    ensureNoteInQueue(
        ...
        const missingDecks = matchedNoteTags.filter((matchedNoteTag) => {
            const reviewDeck = this.reviewDecks.get(matchedNoteTag);
            if (reviewDeck === undefined) return true;

            const inNewNotes = reviewDeck.newNotes.some(
                (newNote) => newNote.path === noteFile.path,
            );
            const inScheduledNotes = reviewDeck.scheduledNotes.some(
                (scheduledNote) => scheduledNote.note.path === noteFile.path,
            );
            return !inNewNotes && !inScheduledNotes;
        });

        if (missingDecks.length > 0) {
            this.addNoteToQueue(noteFile, noteSchedule, missingDecks);
        }
    }
